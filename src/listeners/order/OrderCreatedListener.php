...
class OrderCreatedListener
{
    ...
    protected function toCustomer(Order $order):void
    {
        $sent = $this->mailer
            ->compose(
                ['html' => 'RunShop/order/customer/created-html', 'text' => 'RunShop/order/customer/created-text'],
                ['order' => $order]
            )
    ...
    protected function toAdmin(Order $order): void
    {
        ...
        $sent = $this->mailer
            ->compose(
                ['html' => 'RunShop/order/admin/created-html', 'text' => 'RunShop/order/admin/created-text'],
                ['order' => $order]
            )
            ->setTo($admin_email)
