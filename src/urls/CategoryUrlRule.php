...
namespace Besnovatyj\RunShop\urls;
...
class CategoryUrlRule extends BaseObject implements UrlRuleInterface
{
    ...
    public function parseRequest($manager, $request)
    {
        if (preg_match('#^' . $this->prefix . '/(.*[a-z])$#is', $request->pathInfo, $matches)) {
            ...
            if ($path != $result['path']) {
                throw new UrlNormalizerRedirectException(['/RunShop/catalog/category', 'id' => $result['id']], 301);
            }

            return ['/RunShop/catalog/category', ['id' => $result['id']]];
        }
        return false;
    }
    ...
    {
        $chunks = explode('/', $path); // because slug is unique
        return end($chunks);
    }

    private function getCategoryPath(Category $category): string
    {
        $chunks = ArrayHelper::getColumn($category->getParentsList(), 'slug');
    ...
    {
        if ($route == '/RunShop/catalog/category') {
            if (empty($params['id'])) {
